...
Orchestra has multiple deployable units that share code:
- `packages/core` — business logic shared by API, Tauri, and potentially a CLI
...
- `apps/desktop` — wires adapters for Tauri
...
│   │
│   └── desktop/                    ← Deployable: Tauri app
│       ├── package.json
│       ├── src-tauri/              ← Rust (Tauri core)
│       └── src/wire.ts            ← Wires SqliteAdapter + InMemory + IPC
│
└── docs/
    ├── adr/
    ├── prd/
