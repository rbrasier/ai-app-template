...
The previous codebase had platform-specific code (Electron APIs, PostgreSQL drivers, polling timers) mixed throughout business logic services. This made it impossible to:
...
│            packages/adapters                 │
│                                             │
│  PostgresAdapter   BullMQAdapter            │
│  SqliteAdapter     InMemoryQueueAdapter      │
│  SocketIOAdapter   ElectronIPCAdapter        │
└─────────────────────────────────────────────┘
                 ▲ wires together
┌────────────────┴────────────────────────────┐
│               apps/                          │
│  web-server/index.ts  (wires Postgres, BullMQ, SocketIO)   │
│  desktop/wire.ts      (wires SQLite, InMemory, IPC)        │
└─────────────────────────────────────────────┘
```
...
└── desktop/        ← Wires platform adapters. Entry point for Tauri.
```
